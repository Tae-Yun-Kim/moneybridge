...
public class ContractDTO {
    ...
    // ✅ 요청용 필드 추가
    private Long contractId;  // ✅ 계약 ID (요청 시 사용)
    private String userId;    // ✅ 사용자 ID (요청 시 사용)
    private Boolean isLender; // ✅ 출자자 여부 (요청 시 사용)
    ...
    private String deletedByUsers; // 계약서 삭제한 유저
    ...
    // ✅ 연장 관련 필드 추가
    private boolean extendRepaymentRequested; // 상환 연장 요청 여부
    private Integer extendedPeriod; // 연장된 기간 (개월 수)

    // 🆕 **연장 요청 횟수 필드 추가 (최대 2회)**
    private int extensionRequestCount;
    ...
    public ContractDTO(Contract contract) {
        ...
        this.deletedByUsers = contract.getDeletedByUsers();
        ...
        // ✅ 연장 관련 필드 추가
        this.extendRepaymentRequested = contract.isExtendRepaymentRequested();
        this.extendedPeriod = contract.getExtendedPeriod();

        // 🆕 **연장 요청 횟수 추가**
        this.extensionRequestCount = contract.getExtensionRequestCount();
    }
    ...
    public static ContractDTO toDTO(Contract contract) {
        return ContractDTO.builder()
                ...
                .extendRepaymentRequested(contract.isExtendRepaymentRequested())
                .extendedPeriod(contract.getExtendedPeriod())
                // 🆕 **연장 요청 횟수 추가**
                .extensionRequestCount(contract.getExtensionRequestCount())
                .build();
    }
}
