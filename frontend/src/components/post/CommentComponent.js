...
const CommentComponent = ({ postId, loanAmount, repaymentPeriod }) => {
  ...
  const [postAuthorId, setPostAuthorId] = useState(""); // 🔹 게시글 작성자 ID 저장
  ...
  // const getTrafficLightEmoji = (memberGrade) => {
  //   switch (memberGrade) {
  //     case "GOLD":
  //       return "🔵"; // 골드 = 파란불
  //     case "SILVER":
  ...
  // const getTrafficLight = (memberGrade) => {
  //   return (
  //     <div className="traffic-light">
  //       <div
  //         className={memberGrade === "BRONZE" ? "light red" : "light off"}
  //       ></div>
  //       <div
  //         className={memberGrade === "SILVER" ? "light yellow" : "light off"}
  //       ></div>
  //       <div
  //         className={memberGrade === "GOLD" ? "light green" : "light off"}
  //       ></div>
  //     </div>
  //   );
  // };
  const getRankIcon = (memberGrade) => {
    switch (memberGrade) {
      case "GOLD":
        return "🐓"; // 골드 = 닭
      case "SILVER":
  ...
  useEffect(() => {
    ...
    fetchUserInfo();
    fetchPostAuthor(); // 🔹 게시글 작성자 정보 가져오기
    fetchComments();
  ...
  const fetchPostAuthor = async () => {
    try {
      const postData = await getLoanPostById(postId);
      console.log("postData :", postData);
      setPostAuthorId(postData.writerId);
    } catch (error) {
      console.error("게시글 작성자 정보를 가져오는 중 오류 발생:", error);
    }
  };
        ...
        {comments.map((comment) => (
          <div key={comment.id} className="comment-item-c">
            <div className="comment-header-c">
              ...
              >
                <span>
                  대출 희망자 ID: <b>{comment.memberId}</b>
                </span>
                {/* {getTrafficLight(comment.memberGrade)} */}
                <div className="chicken">
                  {getRankIcon(comment.memberGrade)}
                </div>
              </div>
            ...
            <div className="button-container-c">
              ...
              {/* 🔹 게시글 작성자만 선택 버튼 보이도록 조건 추가 */}
              {userInfo.userId === postAuthorId && (
                <button
                  className="select-button-c"
